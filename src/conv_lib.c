...
malloc_multi (1,1,sizeof(float),  (*masksize)+1,0,0, mask);
...
malloc_multi (1,2,sizeof(float),  nx+masksize+masksize,UNROLL,0,0,0,0, &help);

/* allocate storagy for that many results */
malloc_multi (1,1,sizeof(float), UNROLL,0,0, &sum);
...
free_multi (1,2,sizeof(float),  nx+masksize+masksize,UNROLL,0,0,0,0, &help);
free_multi (1,1,sizeof(float), UNROLL,0,0, &sum);
...
malloc_multi(1,1,sizeof(float), ny+masksize+masksize,0,0, &help);
...
free_multi(1,1,sizeof(float), ny+masksize+masksize,0,0, &help);
